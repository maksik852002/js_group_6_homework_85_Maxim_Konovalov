...
export const addTrackToHistory = track => {
  return async (dispatch, getState) => {
    try {
      ...
      const response = await axios.post('/track_history', {track}, user && {headers: {'Authorization': 'Token ' + user.token }});
...
export const fetchTrackHistory = () => {
  return async (dispatch, getState) => {
    try {
      ...
      const response = await axios.get('/track_history', user && {headers: {'Authorization': 'Token ' + user.token }});
